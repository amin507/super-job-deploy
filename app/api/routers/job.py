...
@router.post(
    ...
    summary="Create Job Position",
    description="""
    Membuat posisi pekerjaan baru dengan SEMUA field yang tersedia.
    
    **Request Body:** Semua field dari JobCreate schema
    
    ### Step 1: Informasi Dasar Pekerjaan
    - `title` (required): Judul posisi
    - `job_code` (optional): Kode job unik (auto-generated jika tidak disediakan)
    - `department`: Departemen
    - `location`: Lokasi
    - `employment_type`: Jenis pekerjaan (Full-time, Part-time, Contract)
    - `experience_level`: Level pengalaman
    ...
    ### Informasi Gaji:
    - `salary_min`, `salary_max`: Range gaji
    - `salary_currency`: Mata uang (default: IDR)
    - `salary_interval`: Interval (hourly, daily, weekly, monthly, yearly)
    - `salary_range`: Display text untuk range gaji
    
    ### Rentang Usia:
    - `min_age`, `max_age`: Usia minimal dan maksimal (17-65)
    
    ### Step 2: Persyaratan dan Deskripsi
    - `description`: Deskripsi pekerjaan
    - `responsibilities`: Tanggung jawab
    - `qualifications`: Kualifikasi
    ...
    ### Step 3: AI Interview Settings (Opsional)
    - `ai_interview_enabled`: Aktifkan AI interview
    - `ai_interview_questions_count`: Jumlah pertanyaan (1-20)
    - `ai_interview_duration_seconds`: Durasi (30-600 detik)
    - `ai_interview_deadline_days`: Batas waktu (1-30 hari)
    - `ai_interview_questions`: Daftar pertanyaan (rich text)
    
    ### Lainnya:
    - `status`: Status awal (default: draft)
    - `contact_url`: URL kontak
    ...
    **Contoh Request Body:**
    ```json
    {
        ...
        "max_age": 40,
        "description": "Mengembangkan aplikasi web...",
        "responsibilities": "1. Develop features\\n2. Code review",
        "qualifications": "1. Bachelor degree\\n2. 3+ years experience",
        "benefits": "BPJS, THR, Bonus",
